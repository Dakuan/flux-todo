...
_           = require 'lodash'
TodoService = require '../../src/coffee/services/todo-service'
...
describe 'TodoService', ->
  ...
  describe 'updateAll', ->
    beforeEach ->
      _.each ['one', 'two', 'three'], (text) -> TodoService.create(text)

    it 'updates all toods', ->
      TodoService.updateAll({ complete: true })
      _.each TodoService.getAll(),
        (todo) -> expect(todo.complete).toEqual(true)
